...
def main():
    ...
    parser.add_argument('--InputDirSM', default = "/user/smoortga/Analysis/MG5_aMC_v2_6_0/CONVERTED_DELPHES_ttbb_TRAINING", help='path to the converted delphes training files')
    parser.add_argument('--InputDirEFT', default = "/user/smoortga/Analysis/MG5_aMC_v2_6_0/CONVERTED_DELPHES_ttbb_ForTemplatesPerCouplings", help='path to the converted delphes training files')
    ...
    couplings = set([f.split("_")[0] for f in os.listdir(args.InputDirEFT) if ".root" in f])
    coupl_strengths = set([f.split("_")[1] for f in os.listdir(args.InputDirEFT) if ".root" in f])
    for c in couplings:
        files_dict[c]={}
