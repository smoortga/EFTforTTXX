...
#     summary_restrict_files.write(args.InputDir+"/restrict_"+param+"_3p0"+".dat \n")
    ...
    runfile.write("generate p p > t t~ b b~ QED=0 DIM6=1 QCD=2, (t > w+ b, w+ > l+ vl), (t~ > w- b~, w- > l- vl~); output MODELSCAN_ttbb_DiLepton_Training_fixed/%s/%s \n"%(param,param+"_3p0_EFT"))
    runfile.close()
    ...
    runfile.write("define j = p \n")
    #runfile.write("generate p p > t t~ c c~ , (t > w+ b, w+ > l+ vl), (t~ > w- b~, w- > l- vl~) QED=0; output MODELSCAN_ttcc_dilepton/%s/%s \n"%(param,param+str(val).replace(".","p").replace("-","minus")))
    runfile.write("generate p p > t t~ b b~ QED=0 DIM6=0 QCD=4, (t > w+ b, w+ > l+ vl), (t~ > w- b~, w- > l- vl~); output MODELSCAN_ttbb_DiLepton_Training_fixed/%s/%s \n"%(param,param+"_3p0_SM"))
    runfile.close()
