...
for param in params:
    ...
    os.system("cp %s/%s %s/restrict_%s1p0.dat"%(args.InputDir,args.ExampleParamCard,args.InputDir,param))
    restrictfile = open(args.InputDir+"restrict_"+param+"1p0"+".dat","r")
    lines = restrictfile.readlines()
    restrictfile.close()


    restrictfile = open(args.InputDir+"/restrict_"+param+"1p0"+".dat","w")
    for line in lines:
    ...
    print "wrote %s"%(args.InputDir+"/restrict_"+param+"1p0"+".dat")
    summary_restrict_files.write(args.InputDir+"/restrict_"+param+"1p0"+".dat \n")
    
    
    for val in orders_to_scan:
        ...
        runfile.write("generate p p > t t~ c c~ QED=0 NP^2==%i; output MODELSCAN_ttcc_inclusive_perOrder/%s/%s \n"%(2*val,param,param+"1p0_Order"+str(val)))
        runfile.close()
        
        print "wrote %s"%(args.InputDir+"/run_"+param+"1p0_Order"+str(val)+".txt")
        summary_run_files.write(args.InputDir+"/run_"+param+"1p0_Order"+str(val)+".txt \n")
