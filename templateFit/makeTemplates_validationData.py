...
def main():
    ...
    parser.add_argument('--InputDir', default = "/user/smoortga/Analysis/MG5_aMC_v2_6_0/CONVERTED_DELPHES_ValidationPerCoupling_WithInterference", help='path to the converted delphes training files')
    ...
    couplings = set([f.split("_")[0][:5] if f.split("_")[0][1:3] in ["83","81","13","11"] else f.split("_")[0][:4] for f in os.listdir(args.InputDir) if ".root" in f])
    coupl_strengths = set([f.split("_")[0][5:] if f.split("_")[0][1:3] in ["83","81","13","11"] else f.split("_")[0][4:] for f in os.listdir(args.InputDir) if ".root" in f])
    for c in couplings:
        validation_files[c]={}
