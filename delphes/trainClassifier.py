...
def makeROC(fpr, tpr, thresholds,AUC,outfile):
...
def main():
    ...
    drawTrainingCurve(args.InputDir + "/training_output/loss_and_acc.pkl",args.InputDir + "/training_output/training_curve.png")
    ...
    makeROC(fpr, tpr, thres,AUC,args.InputDir + "/training_output/roc_SMvsSinglet.png")
    ...
    makeROC(fpr, tpr, thres,AUC,args.InputDir + "/training_output/roc_SMvsOctet.png")
    ...
    Singlet_discr = [j/(discr_dict[1][jdx]+discr_dict[2][jdx]) for jdx,j in enumerate(discr_dict[2]) if y_test[jdx] ==1]
    Octet_discr = [j/(discr_dict[1][jdx]+discr_dict[2][jdx]) for jdx,j in enumerate(discr_dict[2]) if y_test[jdx] ==2]
    fpr, tpr, thres = roc_curve(np.concatenate((np.zeros(len(Singlet_discr)),np.ones(len(Octet_discr)))),np.concatenate((Singlet_discr,Octet_discr)))
    AUC = 1-roc_auc_score(np.concatenate((np.zeros(len(Singlet_discr)),np.ones(len(Octet_discr)))),np.concatenate((Singlet_discr,Octet_discr)))
    makeROC(fpr, tpr, thres,AUC,args.InputDir + "/training_output/roc_SingletvsOctet.png")
